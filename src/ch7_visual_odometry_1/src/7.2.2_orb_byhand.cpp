 ...
 */
void OrientedFAST(const cv::Mat &img, vector<cv::KeyPoint> &keypoints, int threshold,
 ...
 */
void RotatedBRIEF(const cv::Mat &img, vector<cv::KeyPoint> &keypoints,
    ...
    for (auto &kp : keypoints)
    {
        ...
        for (int i = 0; i < 8; ++i)  // 每循环计算32位
        {
            ...
            for (int j = 0; j < 32; ++j)
            {
                ...
                // 逻辑运算
                if (img.at<uchar>(py_rotated, px_rotated) <
                    img.at<uchar>(qy_rotated, qx_rotated))
                    d |= 1 << j;
            }
            desc.bits[i] = d;
        }
 ...
 * @param matches 匹配结果
 */
void BruteForceMatch(vector<DescType> &desc1, vector<DescType> &desc2,
                     vector<cv::DMatch> &matches)
{
    const int d_max =
        40;  // 最大汉明距离, 描述子之间距离小于这个值，才被认为正确匹配
    for (size_t queryIdx = 0; queryIdx < desc1.size(); ++queryIdx)
    {
        cv::DMatch m{int(queryIdx), 0,
                     256};  // 初始化DMatch{queryIdx, trainIdx, distance}
        for (size_t trainIdx = 0; trainIdx < desc2.size(); ++trainIdx)
        {
            ...
            for (int k = 0; k < 8; ++k)
            {
                // desc1[queryIdx].bits[k] ^ desc2[trainIdx].bits[k]
                // 计算两个uint32_t的异或 SSE指令集中的 __mm_popcnt_u32()
                // 计算一个uint32_t中1的个数
                distance += _mm_popcnt_u32(desc1[queryIdx].bits[k] ^
                                           desc2[trainIdx].bits[k]);
            }
            if (distance <
                m.distance)  // 如果有与desc1中的描述子更接近的描述子，更新匹配结果
            {
                m.distance = distance;
