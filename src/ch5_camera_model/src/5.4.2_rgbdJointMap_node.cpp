...
public:
    Pointcloud2Publisher(const vector<cv::Mat>& colorImgs_, 
                        ...
                        :Node("pointcloud2_pub_node"),viewer_("PCL Viewer")
    {
        /*================================= ROS2相关 ===============================*/
        pointcloud2_publisher_ = this->create_publisher<sensor_msgs::msg::PointCloud2>("pointcloud2_pub", 100);
        timer_ = this->create_wall_timer(50ms, bind(&Pointcloud2Publisher::timer_callback, this));
        ...
        /*========================== 使用pcl进行可视化 ============================*/
        // pcl::visualization::CloudViewer viewer_("PCL Viewer");   // 改为类的成员变量,为了退出构造时,CloudViewer不退出
        viewer_.showCloud(cloud_ptr);
...
int main(int argc, char** argv)
{
    ...
    for (int i = 1; i < 6; i++)
    {
        ...
        {
            cerr << "读取图像" << path_color << "或" << path_depth << "出错, 当前路径为" << filesystem::current_path() << endl;
            return EXIT_FAILURE;
        }
    ...
    if(!infile)
    {
        cerr << "读取姿态文件:" << pose_path << "出错, 当前路径为" << filesystem::current_path() << endl;
        return EXIT_FAILURE;
    }
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        poses.push_back(Sophus::SE3d(Eigen::Quaterniond(qw, qx, qy, qz), Sophus::Vector3d(tx, ty, tz)));
    }
    // 创建ROS2节点,传入数据
